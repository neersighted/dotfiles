...
# login actions (in the root shell)
if status --is-login
  # load system profile
  test -f /etc/profile
...
# interactive login actions (root shell when we have a user)
...
# interactive actions (every run with a user)
if status --is-interactive
  ...
  command -s rbenv >/dev/null 2>&1; and set -l rbenv 1
    and fundle plugin 'fisherman/rbenv' --url 'https://github.com/neersighted/rbenv/#patch-1'
